...
static char* semaphore1_file;
static char* semaphore2_file;
...
int main(int argc, char** argv)
{
  ...
  fprintf(stderr, "executor running %s %s\n", argv[1], argv[2]);
...
#if SYZ_EXECUTOR_USES_SHMEM
  // GABE read input from file
  //FILE* input_f = fopen("/tmp/prog.data", "r");
  FILE* input_f = fopen(argv[1], "r");
  size_t rv = fread(input_data, sizeof(char), kMaxInput, input_f);
  if (rv < 0) perror("err reading prog data: ");
  fclose(input_f);

  // GABE: cmdline file to write 1 to before executing
  semaphore1_file = argv[2];
  semaphore2_file = argv[3];
  ...
  // GABE:
    ...
    /* GABE put in simplified execute here */
  }
...
void execute_one()
{
  ...
  // GABE: instead of tracing, wait on semaphore
  fprintf(stderr, "executor opening %s\n", semaphore1_file);
  FILE* barrier_f = fopen(semaphore1_file, "r");
  fgetc(barrier_f);

  if (fclose(barrier_f))
    perror("error closing barrier file: ");


  // GABE: for semaphore tracing we set pid list in scripts, not here
  //int pid_t  = getpid();

  //FILE* pid_ctrl = fopen("/sys/kernel/debug/tracing/set_event_pid", "w");
  //if (pid_ctrl == NULL)
    //perror("could not open set_event_pid");

  //fprintf(pid_ctrl, "%d", pid_t);
  ////if (fputc('1', pid_ctrl) == EOF)
    ////perror("writing to trace_on");

  //if (fclose(pid_ctrl))
    //perror("error closing pid control file: ");


  // GABE: turn on tracing once everyone past semaphore
  FILE* trace_ctrl = fopen("/sys/kernel/debug/tracing/tracing_on", "a");
  if (trace_ctrl == NULL)
    perror("could not open trace_on");

  if (fputc('1', trace_ctrl) == EOF)
    perror("writing to trace_on");

  if (fclose(trace_ctrl))
    perror("error closing trace control file: ");

  fprintf(stderr, "turned on tracing\n");
...
retry:
    ...
    debug("waiting to finish (should not be here?)\n");
  ...
  fprintf(stderr, "executor opening %s\n", semaphore2_file);
  barrier_f = fopen(semaphore2_file, "r");
  fgetc(barrier_f);
  ...
  /* GABE: disable tracing when done */
  trace_ctrl = fopen("/sys/kernel/debug/tracing/tracing_on", "w");
  if (trace_ctrl == NULL)
    perror("could not open trace_on");

  if (fputc('0', trace_ctrl) == EOF)
    perror("writing 0 to trace_on");

  if (fclose(trace_ctrl))
    perror("error closing trace control file: ");


  fprintf(stderr, "turned off tracing\n");

  // remove my pid from pid list


  FILE* watchpt_ctrl = fopen("/kconcur/watchpoint_is_set", "w");
  if (watchpt_ctrl == NULL)
    perror("could not open trace_on");

  if (fputc('1', watchpt_ctrl) == EOF)
    perror("writing 1 to watchpoint_is_set");

  if (fclose(watchpt_ctrl))
    perror("error closing watchpt_ctrl control file: ");
...
void execute_call(thread_t* th)
{
	...
	debug("#%d [%llums] <- %s=0x%llx errno=%d ",
	      th->id, 0, call->name, (uint64)th->res, th->reserrno);
	debug("\n");
}
...
{
  // GABE
  printf("in kcov_comparison_t::ignore()\n");
  return true;
