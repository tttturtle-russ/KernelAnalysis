...
void execute_one()
{
  ...
  // GABE: instead of tracing, write to barrier file
  fprintf(stderr, "executor opening %s\n", barrier_file);
  FILE* barrier_f = fopen(barrier_file, "w");
  if (fputc('1', barrier_f) == EOF)
    perror("writing to barrier file");

  if (fclose(barrier_f))
    perror("error closing barrier file: ");


  // GABE: only trace our current pid
  int pid_t  = getpid();
