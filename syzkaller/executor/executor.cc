...
void execute_one()
{
  ...
  // GABE: debugging check
  /*
  trace_ctrl = fopen("/sys/kernel/debug/tracing/tracing_on", "r");
  if (trace_ctrl == NULL)
    fail("could not open trace_on");

  int tracing_on = fgetc(trace_ctrl);
  printf("tracing_on = %d\n", tracing_on);
...
retry:
  ...
  trace_ctrl = fopen("/sys/kernel/debug/tracing/tracing_on", "a");
  if (trace_ctrl == NULL)
    perror("could not open trace_on");
