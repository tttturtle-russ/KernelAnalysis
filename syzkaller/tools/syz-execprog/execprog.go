...
var (
	...
	flagSerialize       = flag.Bool("serialize", false, "serialize input programs and quit")
	flagSerializeOutput = flag.String("serialize_outputdir", "./", "output dir for serialized programs")
	flagDeserialize		= flag.Bool("deserialize", false, "deserialize input programs and quit")
	flagDeserializeOutput = flag.String("deserialize_outputdir", "./", "output dir for deserialized programs")
)
...
func main() {
	...
	if *flagDeserialize {
		deserializePrograms(target, flag.Args())
		return
	}
	...
	if *flagSerialize {
		serializePrograms(entries, flag.Args())
		return
	}
...
func serializePrograms(entries []*prog.LogEntry, files []string) {

	for i, entry := range entries {
		var fname = filepath.Base(files[i])
		var serializedProg []byte = make([]byte, prog.ExecBufferSize)
		progSize, err := entry.P.SerializeForExec(serializedProg)

    var compactSerializedProg []byte = serializedProg[:progSize]

		if err != nil {
			log.Fatalf("failed to serialize program %v: %v", fname, err)
		}
		os.WriteFile(*flagSerializeOutput+fname+".data", compactSerializedProg, 0644)
		//os.WriteFile(*flagSerializeOutput+fname+".data", serializedProg, 0644)
	}
	log.Logf(0, "serialized %v files to %v", len(files), *flagSerializeOutput)

}

func deserializePrograms(target *prog.Target, files []string) {
	for _, file := range files {
		var fname = filepath.Base(file)
		data, err := ioutil.ReadFile(file)
		if err != nil {
			log.Fatalf("failed to read program %v: %v", fname, err)
		}
		execProg, err := target.DeserializeExec(data)
		if err != nil {
			log.Fatalf("failed to deserialize program %v: %v", fname, err)
		}
		execProg.Dump()
	}	
}
